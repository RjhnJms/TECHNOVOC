...
export default function SMSTab() {
  ...
  const getCourseName = (courses: AssessmentEntry["courses"]): string => {
    if (!courses) return "Unknown"
    if (Array.isArray(courses)) return courses[0]?.course_name || "Unknown"
    return courses.course_name || "Unknown"
  }
  ...
  const buildMessage = async (student: Student): Promise<string> => {
    const { data: assessments } = await supabase
      .from("assessments")
      .select("score, total_items, courses(course_name)")
      .eq("student_id", student.id)

    if (!assessments || assessments.length === 0) {
      return `Hi ${student.full_name}! You have not taken the TECHNO-VOC assessment yet. Please log in at your school to take the assessment. - Northern Antique Vocational School - NAVS`
    }

    const totalScore = assessments.reduce((sum, a) => sum + (a.score || 0), 0)
    const totalItems = assessments.reduce((sum, a) => sum + (a.total_items || 0), 0)
    const scorePercentage = totalItems > 0 ? Math.round((totalScore / totalItems) * 100) : 0

    const { data: rankings } = await supabase
      .from("rankings")
      .select("score, courses(course_name)")
      .eq("student_id", student.id)
      .order("score", { ascending: false })
      .limit(3)

    // If rankings exist use them; otherwise fall back to top 3 from assessments by percentage
    let top3Courses: string[]
    if (rankings && rankings.length > 0) {
      top3Courses = rankings.map((r, index) => `${index + 1}. ${getCourseName(r.courses as any)}`)
    } else {
      const derived = [...assessments]
        .filter(a => (a.total_items || 0) > 0)
        .sort((a, b) => (b.score / b.total_items) - (a.score / a.total_items))
        .slice(0, 3)
      top3Courses = derived.map((a, index) => `${index + 1}. ${getCourseName(a.courses as any)}`)
    }

    const top3 = top3Courses.join(", ")

    return `Hi ${student.full_name}! You completed TECHNO-VOC with a total score of ${totalScore}/${totalItems} (${scorePercentage}%)! Top 3 course recommendations: ${top3}. Collect your printed results on ${collectionDate} at ${collectionLocation}. - NAVS`
  }
      ...
      {showSettings && (
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "24px", border: "1px solid #e5e7eb", marginBottom: "16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            ...
            <div>
              <label style={labelStyle}>Result Collection Date & Time</label>
              <input
                placeholder="e.g., May 24, 2026, 8:00 AM - 4:00 PM"
                value={collectionDate}
                onChange={e => setCollectionDate(e.target.value)}
                style={inputStyle}
              />
              <p style={{ fontSize: "12px", color: "#6b7280", marginTop: "4px" }}>
                When students should pick up their printed results
              </p>
            </div>
            <div>
              ...
              />
              <p style={{ fontSize: "12px", color: "#6b7280", marginTop: "4px" }}>
                Where students should collect their results
              </p>
            </div>
          </div>
          <div style={{ backgroundColor: "#f0fdf4", borderRadius: "8px", padding: "12px", marginTop: "12px" }}>
            ...
            <p style={{ color: "#166534", fontSize: "12px", margin: 0, fontStyle: "italic" }}>
              {`"Hi [Student Name]! Congrats on passing TECHNO-VOC with a total score of 97/100 (97%)! Top 3 course recommendations: 1. Food Tech, 2. ICT, 3. Agriculture. Collect your printed results on ${collectionDate} at ${collectionLocation}. - NAVS"`}
            </p>
          </div>
        </div>
      )}
